...
describe('retro cession allocation engine', () => {
  ...
  it('run allocates exactly 25% of the deposit premium onto the retro contract', async () => {
    ...
    expect(run.json().allocated).toBeGreaterThanOrEqual(1);
  ...
  it('a claim CASH_LOSS event allocates under a CLAIM rule', async () => {
    ...
    const run = await app.inject({ method: 'POST', url: '/api/retrocession/allocation/run', headers: auth, payload: {} });
    expect(run.statusCode).toBe(200);
    // Exactly one new allocation: the CLAIM rule × the CASH_LOSS event. The
    // PREMIUM rule does not apply to claims, and everything else is unchanged.
    expect(run.json().allocated).toBe(1);
    expect(run.json().totalByCurrency).toEqual({ USD: CASH_LOSS_MINOR * 0.3 });
